...
public class WaveAnimationView: UIView {
  ...
  open func startAnimation() {
    if ( !self.isAnimating )
    {
      self.timer = Timer.scheduledTimer(timeInterval: 0.035, target: self, selector: #selector(waveAnimation), userInfo: nil, repeats: true)
  ...
  private func drawWave(layer: CAShapeLayer,path: UIBezierPath,color: UIColor,delay:CGFloat) {
    ...
    if ( layer == frontWaveLayer || self.gradientToColor == nil )
    {
    ...
    if ( delay != 0 && self.gradientToColor != nil )
    {
      ...
      let gy = 1.0 - CGFloat(self.progress);
      /*if ( gy == 0 )
       {
       gy = gy - 0.05
       }*/
      gradientSubLayer.startPoint = CGPoint(x: 0, y: gy)
